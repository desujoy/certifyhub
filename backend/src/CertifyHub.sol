...
contract CertifyHub {
    ...
    function generateCertificate(string memory _candidate_name, string memory _template) external {
        ...
        require(bytes(certificates[_certificate_id].candidate_name).length == 0, "Certificate with this ID already exists");

        Certificate memory cert = Certificate({candidate_name: _candidate_name, template: _template});
    ...
    function generateMultiCertificate(string[] memory _candidate_name, string memory _template) external {
        for (uint256 i = 0; i < _candidate_name.length; i++) {
            ...
            if (bytes(certificates[_certificate_id].candidate_name).length != 0) {
                revert();
            }

            Certificate memory cert = Certificate({candidate_name: _candidate_name[i], template: _template});
            certificates[_certificate_id] = cert;
    ...
    function getCertificate(bytes32 _certificate_id) external view returns (string memory _candidate_name, string memory _template) {
        Certificate memory cert = certificates[_certificate_id];

        require(bytes(certificates[_certificate_id].candidate_name).length != 0, "Certificate with this ID does not exist");
